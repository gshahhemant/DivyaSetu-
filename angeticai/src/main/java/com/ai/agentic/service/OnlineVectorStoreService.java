...
/**
 * RAG (Retrieval-Augmented Generation) service backed by OpenAI (online).
 *
 * <p>Pipeline:
 * <ol>
 *   <li>Ingest: read every row from each .xlsx/.csv file in C:/google_sheets,
 *       convert to a text segment, embed with OpenAI (text-embedding-3-small),
 *       and store in an in-memory vector store.</li>
 *   <li>Search: embed the user query → find the most semantically similar rows
 *       → pass them as context to gpt-4o → return a natural-language answer.</li>
 * </ol>
 */
@Service
public class OnlineVectorStoreService {

    private static final Logger log = LoggerFactory.getLogger(OnlineVectorStoreService.class);
    ...
    public OnlineVectorStoreService(LocalSpreadsheetService spreadsheetService,
    ...
    public void init() {
        ...
        log.info("OnlineVectorStoreService ready. Chat model: {}, Embedding model: {}",
    ...
    public void clearStore() {
        store = new InMemoryEmbeddingStore<>();
        storeSize.set(0);
        log.info("Online vector store cleared.");
    }

    /**
     * Reads ALL .xlsx/.csv files from the configured folder, embeds every row
     * using OpenAI, and stores them in the vector store (previous contents cleared first).
     */
    ...
    /**
     * Embeds all rows from a single file using OpenAI and adds them to the store
    ...
    /**
     * Embeds the query via OpenAI, retrieves the most similar rows from the vector store,
     * then asks gpt-4o to synthesise a natural-language answer.
     */
    public VectorSearchResponse search(String query,
        ...
        if (storeSize.get() == 0) {
            return new VectorSearchResponse(
                    "The vector store is empty. Please call POST /api/online-vector/ingest first.",
