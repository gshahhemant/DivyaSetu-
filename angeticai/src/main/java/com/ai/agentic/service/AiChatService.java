 ...
 *   2. Ask Ollama to generate a SQL SELECT for the user's prompt.
 *   3. Execute that SQL safely (SELECT-only, row-limited).
 *   4. Ask Ollama to convert the raw rows into a natural-language answer.
 */
@Service
public class AiChatService {
    ...
    public AiChatService(
            ...
            @Value("${ollama.timeout}")  int timeoutSeconds) {
    ...
    public ChatResponse chat(String userPrompt) {
        ...
        try {
            ...
            // Model produced unsafe or invalid SQL — tell the user
